...
export class MediaComponent extends GPUComponent {
        ...
        try {
            switch (this._mediaState.type) {
                ...
                case 'audio':
                    ...
                    break;
                default:
                    throw new Error(`Unsupported media type: ${this._mediaState.type}`);
            }
            
            this._mediaState.loaded = true;
            this._currentRetry = 0;
            
            // Apply effects if GPU available
    ...
    async _loadImage() {
        ...
        // Decode if supported
        if (this._decode && 'decode' in loadedImg) {
    ...
    _createImageElement(img) {
        // Use GPU canvas if available
        if (this.gpu.available && this._offscreen) {
    ...
    _handleError(error) {
        ...
        // Retry logic
        if (this._currentRetry < this._retryAttempts) {
            this._currentRetry++;
    ...
    getMediaInfo() {
        return {
            loaded: this._mediaState.loaded,
