...
export class BrutalComponent extends Component {
    ...
    _initializeState(initialState = {}) {
        this.state = new State(initialState, {
            shared: this.config.useSharedState,
    ...
    connectedCallback() {
        ...
        // Emit mount event with particles if enabled
        if (this.config.useParticles && window.__BRUTAL_DEBUG__?.particleEngine) {
            const rect = this.getBoundingClientRect();
            window.__BRUTAL_DEBUG__.particleEngine.emit({
        ...
        // Register with component monitor if debug enabled
        if (this.config.debug && window.__BRUTAL_DEBUG__?.componentMonitor) {
    ...
    disconnectedCallback() {
        ...
        // Emit unmount event with particles
        if (this.config.useParticles && window.__BRUTAL_DEBUG__?.particleEngine) {
            const rect = this.getBoundingClientRect();
            window.__BRUTAL_DEBUG__.particleEngine.emit({
                x: rect.left + rect.width / 2,
        ...
        // Unregister from component monitor
        if (this.config.debug && window.__BRUTAL_DEBUG__?.componentMonitor) {
    ...
    render() {
        ...
        try {
            ...
            // Emit render event if debug enabled
            if (this.config.debug) {
                window.dispatchEvent(new CustomEvent('brutal:component-render', {
                    detail: {
                        component: this,
                        metrics: {
                            renderTime,
    ...
    _onStateChange(path, newValue, oldValue) {
        ...
        // Emit state change particles if enabled
        if (this.config.useParticles && window.__BRUTAL_DEBUG__?.particleEngine) {
            const rect = this.getBoundingClientRect();
            window.__BRUTAL_DEBUG__.particleEngine.emit({
                x: rect.left + rect.width / 2,
    ...
    // Return event handler mappings for delegation
    eventHandlers() {
        return {};
        // Example:
        // return {
