#!/usr/bin/env node
// BRUTAL V5 - API Surface Validator
// Ensures API compatibility is maintained during purification
...
if (!fs.existsSync(CURRENT_STATE_DIR)) {
  console.error('❌ No current state found. Run document-current-state.mjs first!');
  process.exit(1);
}
