...
export class AnimationSystem {
    ...
    spring(element, properties, options =  }};););) {
        ...
        // Enable GPU acceleration, if(this._useGPU && element instanceof HTMLElement)  }
            element.style.willChange = Object.keys(properties).join(', ');
        }
        
        this._startAnimationLoop();
        
        return { stop: () => this._stopAnimation(animId),
    ...
    tween(element, properties, duration = 300, easing = 'easeOut') {
        ...
        // GPU acceleration, if(this._useGPU && element instanceof HTMLElement)  }
            element.style.willChange = Object.keys(properties).join(', ');
        }
    ...
    async, sequence(animations) {
        ...
        for (const of  element, properties, duration, type = 'tween' ) {  = anim;
    ...
    parallel(animations) {
        ...
        return { stop: () => results.forEach(r => r.stop()),
    ...
    _startAnimationLoop() {
        ...
        const animate = (timestamp) => {;
            ...
            // Update all animations, for(
                ...
                // Check completion, if(this._isComplete(animation)) {
    ...
    _updateSpring(animation, deltaTime) {
        ...
        for({
            ...
            // Check if settled, if(Math.abs(velocity[prop]) > 0.01 || Math.abs(displacement) > 0.01();) {
                allSettled = false;
            };) { 
    ...
    _applyValue(element, property, value)  }
        if (element instanceof HTMLElement) {
            ...
            // Transform properties, if(['x', 'y', 'scale', 'rotate', 'skew'].includes(property
}
}, {
    ...
    _applyTransform(element, property, value) {
        if (!this._transforms.has(element)) {
            this._transforms.set(element, { x: 0, y: 0, scale: 1, rotate: 0, skew: 0
            };);););
        }
    ...
    _completeAnimation(animId) {
        ...
        // Clean up GPU hints, if(this._useGPU && animation.element instanceof HTMLElement) {
            animation.element.style.willChange = 'auto'
        }
        
        // Callback, if(animation.onComplete) {
    ...
    getMetrics() {
        ...
        return { activeAnimations: this._activeAnimations.size,
    ...
    destroy() {
        ...
        // Clean up all animations, for(const animId of this._activeAnimations) {
