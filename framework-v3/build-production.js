...
async function resolveImports(code, baseDir) {
    ...
    // Process re-exports
    for (const match of code.matchAll(exportRegex)) {
...
async function extractCSS(code) {
    ...
    for (const match of code.matchAll(cssRegex)) {
        ...
        // Simple heuristic to detect CSS
        if (cssCandidate.includes('{') && cssCandidate.includes('}') && cssCandidate.includes(':')) {
...
async function buildBundle(config) {
    ...
    // Add CSS if any
    if (css) {
        code = `(function(){const s=document.createElement('style');s.textContent='${css}';document.head.appendChild(s)})();${code}`;
    }
