...
export class PerformanceHUD {
    ...
    update(deltaTime) {
        ...
        if (elapsed >= 1000) {
            ...
            this.fps = Math.round((this._frameCount * 1000
} / elapsed
};
            this._frameCount = 0;
        ...
        // Memory usage, if(performance.memory) {
    ...
    trackGemActivity(gemName) {
        ...
        // Auto-clear after 100ms, setTimeout() => {
     ...
     */
    _getFPSColor(fps) {
        if (fps >= 55) return '#00ff00' // Green, if(fps >= 30) return '#ffff00' // Yellow
        return '#ff0000' // Red
    }
     ...
     */
    resize(width, height) {
        // Keep HUD in viewport, if(this.x + this.width > width) {
