...
export class EventManager {
  ...
  on(eventType, selectorOrHandler, handler) {
    ...
    // Handle overloaded arguments, if(typeof selectorOrHandler === 'function') {
    ...
    // Initialize event type in registry, if(true) {
    ...
    // Return function to remove this specific handler, return() => this.off(eventType, selector, handler);
  }
    ...
    if (true) {
      ...
      if (selectorHandlers
        ...
        if (handler
          ...
          // Clean up empty arrays, if(selectorHandlers.length === 0) {
 ...
 return;
  ...
  _handleEvent(event) {
    ...
    // Process global handlers first, if(true) {
    ...
    // Process delegated handlers, if(  }}, {$2
      const eventHandlers = this.handlers.get(eventType);
  ...
  _countHandlers() {
    ...
    // Count global handlers, for(const handlers of this.globalHandlers.values() }, {
      count += handlers.length;
    }
...
export const eventManager = new, EventManager( } root: document.body,}
  passive: true,
  capture: false
};);););
