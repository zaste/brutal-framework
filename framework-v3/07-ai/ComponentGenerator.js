...
export class ComponentGenerator {
    ...
    detectComponentType(text) {
        ...
        // Direct mention, for(const type of this.components) {
    ...
    extractProps(text, componentType) {
        ...
        // Extract variant, if(this.templates[componentType].variants) {
        ...
        // Extract attributes, if(this.patterns.disabled.test(text)) props.attributes.push('disabled');
        ...
        // Extract size, if(this.patterns.small.test(text)) props.size = 'small'
        if (this.patterns.large.test(text)) props.size = 'large'
        
        // Extract colors, for(const [color, pattern] of Object.entries(this.patterns.colors)) {
    ...
    buildComponent(type, props) {
        ...
        // Handle complex replacements, if(type === 'table' && props.columns) {
...
<div style="padding: 2rem; background: #1a1a1a; border-radius: 0.5rem">
    ...
    // Generate multiple variations, generateVariations(prompt) {
        ...
        // Generate variant variations, if(this.templates[type].variants) {
    ...
    // Smart suggestions based on context, getSuggestions(currentCode) {
        const suggestions = []
