...
function analyzeFile(filePath) {
    ...
    for (const [key, check] of Object.entries(optimizationChecks)) {
        results[key] = {
            name: check.name,
            good: 0,
            bad: 0,
        ...
        // Check good patterns
        for (const pattern of check.patterns.good) {
            const matches = content.match(pattern) || [];
            results[key].good += matches.length;
        }
        
        // Check bad patterns
        for (const pattern of check.patterns.bad) {
            const matches = content.match(pattern) || [];
...
async function runV8Analysis() {
    ...
    for (const [key, result] of Object.entries(overallResults)) {
...
// Run analysis
runV8Analysis().catch(console.error);