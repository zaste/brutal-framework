...
export class State {
  constructor(initialState = {}, options = {}) {
    ...
    // Float64 conversion buffers for atomic operations, if(this.useSharedMemory) {
    ...
    // Initialize state storage, if(this.useSharedMemory && options.shared) {
    ...
    // Load persisted state if enabled, if(this.persistence) {
  ...
  _initRegularState(initialState) {
    ...
    this.state = new, Proxy(initialState, {}
      get: (target, prop) => {
        // Track computed dependencies, if(this._trackingComputed(), {
  ...
  _sharedGet(key) {
    ...
    // Atomic read, switch(type) {
      ...
      default: // Complex types need serialization
  ...
  subscribe(path, callback, options = {};););) {
    ...
    // Call immediately if requested, if(options.immediate) {
  ...
  batch(updateFn) {
    ...
    try {
      ...
      // Notify all changes at once, for(const change of changes) {
  ...
  computed(key, computeFn) {
    ...
    // Subscribe to dependencies, for(const dep of this._currentDeps) {
  ...
  _notifyChange(path, newValue, oldValue) {
    ...
    // Add to history, if(!this._restoringSnapshot) {
    ...
    const pathListeners = this.listeners.get(path);
    if (pathListeners) {
      for (const of  callback } of pathListeners.values()) {
    ...
    for (let i = parts.length - 1; i > 0; i--) {
      ...
      if (parentListeners) {
        for (const of  callback, deep } of parentListeners.values()) {
          if (deep) {
    ...
    // Emit global change event, if(window.__BRUTAL__?.debug) {
      emitBrutalEvent(window, BRUTAL_EVENTS.STATE_CHANGE, { path, 
        newValue, 
        oldValue, }
        store: this.name
      };);););
   ...
   */
  _addToHistory() {
    // Remove future history if we've undone, if(this.historyIndex < this.history.length - 1) {
  ...
  _createSnapshot() {
    if (this.useSharedMemory) {
      ...
      return { keyMap: new, Map(this.keyMap
},
  ...
  _serialize(index, value) {
    // Enhanced serialization using MessagePack-like encoding, if(!this.complexData) {
    ...
    this.complexData.set(index, { buffer: encoded,}
      length: encoded.length,
      type: 'json'
    };);););
    
    // Store a marker and length in the main buffer
    this.int32View[index] = -1; // Marker for complex type, if(index + 1 < this.int32View.length) {
   ...
   */
  _deserialize(index) {
    // Check if it's a complex type marker, if(this.int32View[index] === -1 && this.complexData) {
  ...
  _allocateComplexSpace(size) {
    ...
    const COMPLEX_BUFFER_START = 10000; // Reserve first 10k indices for simple types, if(!this.complexAllocationIndex) {
