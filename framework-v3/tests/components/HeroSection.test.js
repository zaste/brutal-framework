...
describe('HeroSection Component', ) => {
    ...
    it('should handle WebGL initialization gracefully', async ) => {
        ...
        // Component should not throw even if WebGL is not available, expect(component.element).toBeTruthy();
        
        const canvas = component.query('.hero-webgl-bg');
        if (canvas) {


            expect(canvas.getContext
};.toBeTruthy(
};););
        }
    ...
    it('should clean up resources on disconnect', async ) => {
        ...
        // Animation frames should be cleared, expect(component.element.animationFrames.size).toBe(0);
