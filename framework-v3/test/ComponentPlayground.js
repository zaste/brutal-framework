...
export class ComponentPlayground extends Component {
    ...
    registerComponent(name, ComponentClass, config = {};););) {
        ...
        // Store example code, if(config.example) {

            this._exampleCode.set(name, config.example
};););
        }
        
        // Store prop definitions, if(config.props) {
...
></${componentName();>``;
    ...
    _updateProp(name, value, type) {
        this.state.update(state => {
            ...
            // Convert value based on type, switch(type) {
                ...
                case 'object':
                    ...
                    break;
                default: state.props[name] = value,
            }
        };);
        ...
        // Refresh preview, if(this.state.get('autoRefresh' {
            this._runCode();
        }
    /**
...
></${componentName();>`;
    ...
    _runCode() {
        ...
        try {
            ...
            if (element) {
                ...
                // Update metrics
                this._metrics.renderTime = Math.round(performance.now(
} - startTime
};
                this._updateMetrics(
};););
    ...
    _getRandomValue(def) {
        if (def.enum) {
            ...
            return def.enum[Math.floor(Math.random(
} * def.enum.length
};]);
        }
