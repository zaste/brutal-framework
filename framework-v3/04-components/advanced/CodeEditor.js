...
export class CodeEditor extends InteractiveComponent {
    ...
    _getVisibleLines() {
        ...
        return {
            start: Math.max(0, scrollLine - 10),
    ...
    _renderLines(visible) {
        ...
        for (let i = visible.start; i < visible.end; i++) {
            if (this._isLineFolded(i)) continue;
    ...
    _tokenizeLine(line, tokens) {
        ...
        // Strings
        for (const [type, regex] of Object.entries(tokenizers.string)) {
            regex.lastIndex = 0;
        ...
        while ((match = tokenizers.bracket.exec(line)) !== null) {
            ...
            tokens.push({
                type: isMatched ? 'bracket matched' : 'bracket',
                start: match.index,
                end: match.index + match[0].length
            });
        }
    ...
    _isLineFolded(line) {
        ...
        for (const foldedLine of this._folds) {
    ...
    _handleClick(e) {
        ...
        if (action === 'fold') {
            const line = parseInt(e.target.closest('.line-number').dataset.line);
    ...
    _insertNewLine() {
        ...
        if (this._config.autoIndent) {
            ...
            // Additional indent after opening brace
    ...
    _ensureLineRendered(line) {
        // Virtual scrolling - ensure line is rendered
        if (!this._renderCache.has(line)) {
