...
export class Carousel extends InteractiveComponent {
    ...
    static get observedAttributes() {
        return [...super.observedAttributes, 'autoplay', 'autoplay-delay', 'loop', 
    ...
    connectedCallback() {
        ...
        // Set up event delegation for controls after render
    ...
    _initialize() {
        ...
        if (this._config.gpuAcceleration && this._config.webglTransitions) {
            this._initWebGL();
        }
            ...
            cube: `
                ...
                void main() {
                    ...
                    if (u_progress < 0.5) {
                        ...
                        float x = sin(rotation) * u_direction;
    ...
    _setupGestures() {
        ...
        // Defer gesture setup to ensure track exists
        requestAnimationFrame(() => {
    ...
    _onDragMove(event) {
        ...
        if (deltaTime > 0) {
            this._velocity.x = (this._dragCurrent.x - this._dragStart.x) / deltaTime;
        }
    ...
    _onDragEnd(event) {
        ...
        if (this._config.loop) {
            if (targetIndex < 0) targetIndex = this._slides.length - 1;
            if (targetIndex >= this._slides.length) targetIndex = 0;
        } else {
    ...
    _updateSlidesVisibility() {
        ...
        slides.forEach((slide, index) => {
            ...
            // GPU optimization
            if (this._config.gpuAcceleration) {
    ...
    _startUpdateLoop() {
        const update = (timestamp) => {
            ...
            // Update physics
            if (this._config.momentum && this._momentumAnimation) {
                this._updateMomentum();
            }
            
            // Update WebGL
            if (this._config.webglTransitions && this._isTransitioning) {
