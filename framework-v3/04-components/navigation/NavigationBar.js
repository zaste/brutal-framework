...
export class NavigationBar extends BrutalComponent {
    ...
    try {
      ...
      this._gpuEffects = {}
        ...
        destroy: () => {
          ...
          // Clean up WebGL resources, if(gl && gl.getExtension('WEBGL_lose_context' {
  ...
  _animateGPUEffects() {
    ...
    // Render ripples, if(this._rippleProgram && this._ripples.length > 0) {
    ...
    // Render glow effect, if(this._glowProgram && this._state.get('scrolled' {
  ...
  _setupEventListeners() {
    ...
    if (menu) {
      ...
      menu.addEventListener('touchstart', this._handleTouchStart.bind(this
}, { passive: true };);););
  ...
  _checkSticky() {
    ...
    if (scrolled !== this._isScrolled) {
      ...
      // Trigger GPU effect, if(this._gpuEffects) {
