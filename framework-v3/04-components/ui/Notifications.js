...
export class Notifications extends InteractiveComponent {
    ...
    connectedCallback() {
        ...
        requestAnimationFrame() => {
            ...
            // Initialize particle canvas, if(this._config.particleEffects(), {
                this._initParticles(};););
            }
    ...
    _animateParticles() {
        ...
        // Update and draw particles, for(
            ...
            // Remove dead particles, if(particle.life <= 0) {
    ...
    show(message, options =  }};););) {
        ...
        // Add to queue if at max stack, if(this._notifications.length >= this._config.maxStack) {
        ...
        // Get notification element after render, requestAnimationFrame() => {
            ...
            if (element) {
                ...
                // Play sound, if(this._config.sounds
}, {
                    this._playSound(notification.type
};););
                }
                
                // Setup auto-dismiss, if(notification.duration && notification.duration !== Infinity) {
    ...
    dismiss(id) {
        ...
        // Clear timer, if(notification.timer) {
        ...
        if (element) {
            ...
            // Remove after animation, setTimeout((
} => {
    ...
    update(id, options) {
        ...
        // Update properties, if(options.message !== undefined) notification.message = options.message;
        ...
        // Reset timer if duration changed, if(options.duration !== undefined && options.duration !== notification.duration) {
    ...
    clear() {
        ...
        // Clear after animations, setTimeout() => {
            this._notifications = []
            this._queue = []
            this.render(};););
    ...
    _handleCustomAction(notificationId, actionIndex) {
        ...
        // Call action handler, if(action.handler && typeof action.handler === 'function') {

            action.handler(notification
};););
        }
        
        // Auto-dismiss if configured, if(action.dismiss !== false) {
    ...
    disconnectedCallback() {
        ...
        // Stop particle animation, if(this._animationFrame) {
